...
function resumeRtf(r, paper='Letter') {
  ...
  return `{\\rtf1\\ansi\\deff0{\\fonttbl{\\f0 Times New Roman;}}\\paperw${page.twips[0]}\\paperh${page.twips[1]}\\margl691\\margr691\\margt${margin}\\margb${margin}\\fs${fs}\\sl${line}\\slmult1\\qc\\b\\fs${name} ${rtfEscape(r.fullName||'YOUR NAME')}\\b0\\fs${Math.max(15,fs-1)}${contactLines}\\par\\ql ${sections.map(section=>`${String(section.title||'').trim()?`\\sb${Math.round(layout.section*20)}\\sa30\\b\\fs${heading} ${rtfEscape(section.title)}\\b0\\fs${fs}\\brdrb\\brdrs\\brdrw5\\par`:''}${parsedItems(section).map(item=>`\\sb25\\brdrnone\\tqr\\tx${rightTab} ${item.title||item.meta?`\\b ${rtfEscape(item.title)}\\tab ${rtfEscape(item.meta)}\\b0\\line `:''}${rtfEscape(item.content)}\\par`).join('')}`).join('')}}`;
}
