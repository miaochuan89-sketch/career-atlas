...
function resumeLayout(r) {
  ...
  if(score<1150)return {mode:'spacious',margin:.48,font:11,line:1.3,section:9,group:7,name:19,heading:12};
  if(score<2300)return {mode:'standard',margin:.43,font:10.5,line:1.22,section:7,group:5.5,name:18,heading:11.5};
  if(score<3450)return {mode:'compact',margin:.34,font:9.5,line:1.13,section:4.5,group:4,name:17,heading:10.5};
  return {mode:'dense',margin:.28,font:8.5,line:1.06,section:3,group:2.5,name:16,heading:9.5};
}
...
function resumeRtf(r, paper='Letter') {
  ...
  return `{\\rtf1\\ansi\\deff0{\\fonttbl{\\f0 Times New Roman;}{\\f1 Arial;}}\\paperw${page.twips[0]}\\paperh${page.twips[1]}\\margl691\\margr691\\margt${margin}\\margb${margin}\\fs${fs}\\sl${line}\\slmult1\\qc\\b\\fs${name} ${rtfEscape(r.fullName||'YOUR NAME')}\\b0\\fs${Math.max(15,fs-1)}${contactLines}\\par\\ql ${sections.map(section=>`${String(section.title||'').trim()?`\\sb${Math.round(layout.section*20)}\\sa30\\b\\fs${heading} ${rtfEscape(section.title)}\\b0\\fs${fs}\\brdrb\\brdrs\\brdrw3\\brdrcf0\\par`:''}${parsedItems(section).map(item=>`\\sb35\\brdrnone\\tqr\\tx${rightTab} ${item.title||item.meta?`\\b ${rtfEscape(item.title)}\\b0\\tab\\f1\\i\\fs${Math.max(14,fs-2)} ${rtfEscape(item.meta)}\\i0\\f0\\fs${fs}\\line `:''}${rtfEscape(item.content)}\\par`).join('')}`).join('')}}`;
}
