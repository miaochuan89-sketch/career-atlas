...
- 每个简历模块包含可独立新增、删除和排序的二级条目；每个条目分别编辑标题与内容
- 二级条目提供独立的左侧标题、右侧日期/地点和正文输入；右侧信息在预览、PDF 与 Word 中统一右对齐
- 简历纸张可选择 Letter 或 A4；右侧预览、PDF 与 Word 使用同一规格，并根据内容量自动调整排版密度
- 简历页眉采用两级联系方式：地点、电话和邮箱位于主行，LinkedIn 与作品集网站位于更精炼的次行
- 简历条目中只有一项的学位、专业或次级说明保持左对齐；日期等第二列信息保持右对齐
- 简历模块使用更细、更浅的分隔线；正文采用更舒展的字号与行距，并自动利用页面剩余高度铺满整页
- 日期和地点采用更轻的无衬线斜体，降低视觉层级并突出左侧的学校、公司和项目名称
- 简历提供 Professional / ATS、Design Minimal 与 Compact 三套模板，可独立选择字号、行距、模块间距和页边距
- “自动适配一页”可按内容量调整密度并利用页面剩余空间；关闭后使用固定标准排版
- 多方向求职策略视图
...
打开 `outputs/installer/Career-Atlas-Setup-1.17.0.exe`，按提示完成安装。任务计划可通过 JSON 导入与导出，以后调整计划通常不再需要重新安装 App。
